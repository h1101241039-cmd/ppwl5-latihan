...
export const userRepository = {
  findAll(): UserModel[] {
    ...
    return rows.map(u => new UserModel(u));
  },

  create(user: User) {
    db.query("INSERT INTO users (name, role) VALUES (?, ?)").run(user.name, user.role);
  },

  update(id: number, user: User) {
    db.query("UPDATE users SET name = ?, role = ? WHERE id = ?").run(user.name, user.role, id);
  },
